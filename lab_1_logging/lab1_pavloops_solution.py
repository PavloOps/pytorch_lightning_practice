...
def evaluate_model(model, X_test, y_test):
    ...
    cls_report = classification_report(
        y_test, y_pred, target_names=[str(cls) for cls in sorted(y_test.unique())], output_dict=True
    )
...
if __name__ == "__main__":
    ...
    logger.report_table(
        title="Validation", series="Missing Values", table_plot=X_train.isna().sum().reset_index(name="missing_count")
    )

    # TRAIN & SAVE MODEL
    model = train_catboost(
        X_train, y_train, X_test, y_test, config=configuration["catboost_params"], need_verbose=args.verbose
    )
